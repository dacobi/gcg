...
# 3. Process every single pixel for a faster white alpha gradient outward
# Reduced from 160 to 45 for a significantly faster fadeout
FADE_RADIUS = 45.0 

for y in range(height):
    for x in range(width):
        # If the pixel is inside the heart, keep it solid white
        if mask_img.getpixel((x, y)) == 255:
            pixels[x, y] = (255, 255, 255, 255)
        else:
            # Calculate shortest distance to the heart's outer edge
            ...
            # Map the white color value with the falling alpha mask
            if alpha > 0:
                pixels[x, y] = (255, 255, 255, alpha)

# Save directly to your desktop
desktop = os.path.join(os.path.expanduser("~"), "Desktop")
output_path = os.path.join(desktop, "fast_fade_white_heart.png")
img.save(output_path, "PNG")

print(f"Success! White gradient transparent PNG created on your Desktop at: {output_path}")
